...
This project translates natural-language questions about Arizona State University parking into MongoDB geospatial queries. It includes a command-line demo, a localhost web app with light/dark themes, sample parking data, and a Mongo-backed test suite.
...
MONGODB_DATABASE=asu_parking
PORT=8090
HOST=127.0.0.1
```

Override these when you need a different MongoDB database or web server port.
...
├── web_server.js          # Local browser demo and JSON API
...
The command-line app includes an interactive mode where you can:
...
The web app exposes:
- `GET /` for the browser interface
- `GET /api/lots` for all parking lots
- `GET /api/stats` for campus-level totals
- `GET /api/search?q=...` for natural-language search
- `POST /api/update` to simulate availability changes

The interface shows campus totals, availability bars, permit/rate details, EV and ADA indicators, and responsive layouts for desktop and mobile.

### Dark Mode

The browser demo includes a `Dark Mode` toggle in the header. It follows the system color preference on first load, then saves the user's choice in `localStorage` for later visits.
...
Update the ordered `permitTypes` array in `query_parser.js`. Put longer phrases before shorter phrases so specific matches win:
...
1. **MongoDB Connection Error**
   ...
   - Set `MONGODB_URI` if your database runs somewhere else
   - Set `MONGODB_DATABASE` if you want a database name other than `asu_parking`
...
2. **No Results Found**
   ...
   - Run `npm run setup` again to reset the sample lots
...
4. **Web App Port Already in Use**
   - Start the server on another port, for example `PORT=8091 npm run web`
...
5. Expand the web interface with maps, filters, or saved queries
